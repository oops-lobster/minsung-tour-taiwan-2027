...
export function WeatherCard() {
  ...
  const loadWeather = async (signal?: AbortSignal) => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        daily: 'precipitation_probability_max,wind_speed_10m_max,wind_gusts_10m_max',
        forecast_days: '1',
      ...
      const data = await response.json() as {
        ...
        daily: { precipitation_probability_max: number[]; wind_speed_10m_max: number[]; wind_gusts_10m_max: number[] }
      }
      setWeather({
        ...
        updatedAt: new Intl.DateTimeFormat('ko-KR', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Taipei' }).format(new Date()),
      })
      setStatus('ready')
    } catch (error) {
  ...
  const strongWind = weather ? weather.windGust >= 40 || weather.windSpeed >= 30 : false
  ...
  return (
    <article className="tool-card weather-card" aria-labelledby="weather-title">
      <div className="tool-card__heading">
        ...
        <button type="button" onClick={() => void loadWeather()} aria-label="날씨 새로고침">
          <RefreshCw size={18} aria-hidden="true" />
        </button>
      </div>
      {status === 'loading' && <p className="tool-loading" aria-live="polite">타이베이 날씨를 불러오고 있어요.</p>}
      {status === 'error' && (
        <div className="tool-error" role="alert">
          <p>날씨를 불러오지 못했습니다. 네트워크를 확인해 주세요.</p>
          <button type="button" onClick={() => void loadWeather()}>다시 시도</button>
        </div>
      )}
      {weather && status === 'ready' && (
        <>
          <div className="weather-now">
            ...
            <div><CloudRain size={21} aria-hidden="true" /><span>오늘 강수확률</span><strong>{weather.precipitationProbability}%</strong><small>현재 강수 {weather.precipitation}mm</small></div>
            <div><Wind size={21} aria-hidden="true" /><span>바람</span><strong>{weather.windSpeed.toFixed(0)}km/h</strong><small>최대 돌풍 {weather.windGust.toFixed(0)}km/h</small></div>
          </div>
          ...
          <div className="weather-links">
            ...
            <button type="button" onClick={() => document.getElementById('tools-weather-plans')?.scrollIntoView({ behavior: 'smooth' })}>우천 Plan 보기</button>
          </div>
