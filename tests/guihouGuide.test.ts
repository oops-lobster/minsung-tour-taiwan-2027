...
test('Day 2 keeps the Shifen mini guides alongside the Guihou field guide', async () => {
  const guides = await readFile(new URL('../src/data/guides.ts', import.meta.url), 'utf8')
  const day2 = await readFile(new URL('../src/data/itinerary/day2.ts', import.meta.url), 'utf8')
  const localTools = await readFile(new URL('../src/components/LocalToolsView.tsx', import.meta.url), 'utf8')
  assert.match(guides, /shifen-waterfall\.html/)
  assert.match(guides, /shifen-old-street\.html/)
  assert.match(guides, /#guide\/guihou/)
  assert.match(day2, /"guideId": "guihou"/)
  assert.match(localTools, /GuideHub/)
})
